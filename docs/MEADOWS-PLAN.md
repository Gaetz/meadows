...
| 🔨 **Crime & prime** | P1 | v1 FAITE (chantier 6 D2) : témoins (victime ou PNJ < 20 m avec LOS), composant `Bounty` mirroré en tag `Crime.Wanted`, gardes hostiles tant que Wanted, amende payable en dialogue. **Bounty PAR FACTION FAIT (session 2026-07-13)** : la prime va à la faction du TÉMOIN, un garde ne chasse que si SA faction détient une tranche (`bountyToward` — le total non attribué des vieilles saves compte pour tous), persistance `SavedBountyForm` par nom de tag. Reste : amende PAR faction (quand le dialogue d'amende connaît son garde), arrestation/prison. |
