...
| 🔨 **Texturing du terrain** | P0 | **CHANTIER EN COURS (2026-08-02/03) — `docs/TERRAIN-TEXTURING.md`** (brief : `docs/AUDIT/TERRAIN-TEXTURING.md`). FAITS : chantier A (pipeline offline — formats BC7/BC5/R16 + upload de mips RHI Vulkan-only, `cooker cook-terrain-materials`, bibliothèque CC0 5 couches ~8,5 Mo, chargement runtime + VFS, doctests) ; B1 poids isolés (hybride C) ; B2 height blending ; B3 TerrainShadeMap (biome/wetness → GPU en champs continus) ; B4 teinte macro + raccords herbe/GI. **En attente : validation visuelle dev**, puis B5 prototype comparatif (toggle A/B), B6 detail normal, B7 POM, B8 stochastique. |
