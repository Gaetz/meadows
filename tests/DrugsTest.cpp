...
TEST_CASE("drugs: the aftershock hits persistent resonance and releases harmony") {
    GameplayTagRegistry tags;
    tags.registerTag("Status.HarmonyBroken");
    AbilitySystem sys;
    ActiveDrugs drugs;
    ...
    tickDrugs(drugs, persistent, sys, 1.0 * 3600.0, tags); // 1h — still active
    CHECK(drugs.list.size() == 1);
    CHECK(harmonyBroken(sys, tags));
    CHECK(persistent.amber == doctest::Approx(0.0f));

    tickDrugs(drugs, persistent, sys, 2.0 * 3600.0, tags); // +2h — wears off
    CHECK(drugs.list.empty());
    CHECK_FALSE(harmonyBroken(sys, tags));
    CHECK(persistent.amber == doctest::Approx(-30.0f)); // aftershock
}
