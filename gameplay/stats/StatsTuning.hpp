...
// from the FormDatabase by a canonical guid; the defaults are the shipped
// values, so behaviour is unchanged when no tuning Form is present. New stats
// content reads its knobs from here too. Threaded into the derived
// calculators (captured), survival, and damage. Design: docs/STATS.md.
...
struct StatsTuningForm : data::Form {
    ...
    // Barter. Prices = goldValue × these; charisma/speechcraft scaling
    // joins with a later stats pass.
    f32 barterBuyMult { 1.5f };   // the player BUYS at value × this
    f32 barterSellMult { 0.5f };  // the player SELLS at value × this
    // Offensive stats + combat state machine.
    ...
    // Movement & world interaction: the stat-space -> world mapping
    // (docs/STATS.md §3) and the first-person feel constants, moddable.
    // What deliberately STAYS C++ is tagged [cpp-tuning].
    ...
    // Footsteps: the first-person player has no walk anim, so they fire
    // every strideLength meters of grounded travel (NPCs use their
    // clips' AnimEvents).
    f32 strideLength { 2.2f };
    // Directional blocking. A raised guard
    ...
    // Entering Alert shouts — same-faction allies within this radius of
    // the caller join the hunt.
    f32 helpCallRadius { 20.0f };
    // Perfect parry (docs/STATS.md §4): a guard raised at
    ...
    // Dodge move (docs/STATS.md §4 — the 2D arena move in 3D):
    ...
    f32 dodgeSpeedMultiplier { 2.08f }; // hand-tuned
    // STATS.md §4: holding the guard is effortful — energy regen halves
    // while State.Blocking is up.
    ...
    // later. Footstep sounds soften too.
    f32 sneakDetectionFactor { 0.5f };
    f32 sneakVolumeFactor { 0.5f };
    f32 sneakPitchFactor { 0.85f };
    f32 sneakSpeedFactor { 0.75f };
    // Charged bow shot — seconds to a full draw and the speed floor a
    // bare tap still looses at (the drain is the BowDrawCost effect,
    // data).
    ...
    // HUD vitals-bar scale — stat points per 1% of the (half-screen)
    // bar container (5 -> 500 points = full width).
    f32 hudStatPointsScale { 5.0f };
    // Walking within this of a planted arrow recovers it (m).
    f32 arrowPickupRadius { 1.5f };
    // Follow feel (gameplay/actors/Followers.hpp bands;
    // docs/FOLLOWERS.md). Near = stand and face the
    ...
    // Passive affinity growth per game-hour spent traveling together
    // (accrueTimeTogether).
    ...
    f32 ageFloor { 0.5f };
    // Follower special powers.
    // A follower in combat re-tries their power every retry window
    ...
    // Fall damage (docs/STATS.md §4):
    // below minHeight a landing is free; above it, blunt
    ...
    // Sneak attack (docs/STATS.md §4): the multiplier a
