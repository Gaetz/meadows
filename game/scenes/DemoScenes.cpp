...
gameplay::StatModifiers StatsScene::resonanceModifiers() const {
    ...
    const gameplay::Resonance da = gameplay::drugAftereffectResonance(activeDrugs); // N4: aftereffect
    eff.onyx += da.onyx;
    eff.amber += da.amber;
...
void StatsScene::drawUi() {
    ...
    Resonance survContrib = effectiveResonance(resonance, survival, tuning);
    ...
    const Resonance     afflC    = afflictionResonance(afflictions, afflictionDb);
    ...
    const bool      hBroken = harmonyBroken(system, tags);
    ...
    if (ImGui::Button("Advance 6h (game time)")) {
        ...
        auto gtArgs = makeGameTimeArgs();
        // equipmentModifiers only: buildCharacterMods inside advanceGameTime already
        // recomputes resonance/injuries/afflictions — passing resonanceModifiers() here
        // would double-apply them and corrupt maxHealth / attribute offsets.
        const gameplay::StatModifiers equipMods = equipmentModifiers();
