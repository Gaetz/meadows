...
// that logic is decoupled from LandscapeScene. Rebuilt per call
...
struct QuestContext {
    ...
    const data::TextTable& texts;            // Quest.* toast strings
    ...
    // Per-faction crime: resolves an actor entity to its
...
// The quest / crime / dialogue director extracted from LandscapeScene:
// the demo quest state machine (the EasternMenace tree), its mirror
...
public:
    ...
    // Event bodies — the scene's subscriptions call these. The
