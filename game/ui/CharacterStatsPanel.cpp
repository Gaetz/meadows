...
void drawCharacterStatsPanel(flecs::entity player,
    ...
    ImGui::SeparatorText("Status buildup (N1) \xe2\x80\x94 fill to endurance to trigger");
    ...
    ImGui::SeparatorText("Injuries (N2) \xe2\x80\x94 heal by sleeping");
    ...
    ImGui::SeparatorText("Afflictions (N3) \xe2\x80\x94 diseases/psychoses, gated by resonance");
    {
    ...
    if (ImGui::Button("Inflict disease (set amber < 0 first)")) {
        ...
        inflictEffect(vitals, system, demo.sampleDisease, amber, 1.0, demo.rng, ctx.tags);
    }
    ImGui::SameLine();
    if (ImGui::Button("Inflict psychosis (set garnet < 0 first)")) {
        const f32 garnet = currentValueOf(system, attr("garnet"));
        inflictEffect(vitals, system, demo.samplePsychosis, garnet, 1.0, demo.rng, ctx.tags);
    }

    ImGui::SeparatorText("Drugs (N4) \xe2\x80\x94 boost, harmony break, progressive aftershock");
